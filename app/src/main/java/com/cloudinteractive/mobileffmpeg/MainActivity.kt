...
//                    .also { Log.e(TAG, it) }
        ...
        val logoPath = getFileFromAssets(this@MainActivity, "logo.png").absolutePath

        val command =
          "-i $inputVideoFilePath -acodec copy -vf \"movie=$logoPath,colorkey=black:1.0:1.0 [wm];[in][wm]overlay=30:10[out]\" ${newFile.absolutePath.encode()}"
        ...
        withContext(Dispatchers.Main) {
          ...
          if (ReturnCode.isSuccess(session.returnCode)) {
            MediaScannerConnection.scanFile(this@MainActivity, arrayOf(newFile.absolutePath), arrayOf("video/*")) { _, uri ->
              Log.e("Jimi", uri.toString())
            }
      ...
      if (resultCode == Activity.RESULT_OK) {
        data?.data?.let {
          ...
          lifecycleScope.launch(Dispatchers.IO) {
            ...
            val probeSession =
              FFprobeKit.execute("-skip_frame nokey -select_streams v:0 -show_entries frame=pkt_pts_time -of csv=print_section=0 -i $inputVideoFilePath")
